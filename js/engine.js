...
populateCreator() {
  ...
  resolveTileTriggers(actor, x, y) {
    ...
    if (tile.encounter && !this.state.combat.active) this.startEncounter(tile.encounter);
  }
...
interactWithActor(id) {
    ...
    if (!actor) return;
    this.state.selectedActorId = id;
    if (actor.role === 'enemy' && !this.state.combat.active) return this.startCombat();
    if (actor.dialogueId) {
      this.openDialogue(actor.dialogueId, actor);
      return;
    }
    this.inspectActor(actor);
  }
  ...
  inspectTile(x, y) {
    const tile = this.currentMap().tiles[y][x];
    let html = `<div class="card"><strong>Tile (${x}, ${y})</strong><div class="small">${tile.type}</div>`;
    if (tile.interactText) html += `<p>${tile.interactText}</p>`;
    if (tile.lootTable?.length) {
      html += `<p><strong>Loot:</strong></p>`;
...
attack(attacker, target, ability = null) {
  ...
  startEncounter(encounterId) {
    ...
    if (!enc) return;
    const selectedNode = getById(this.data.encounters, encounterId);
...
runAITurn(actor) {
    ...
    if (distance(actor, target) <= 1) this.attack(actor, target);
    else this.moveActorToward(actor, target.x, target.y);
  }
  ...
  travelToSector(nodeId) {
    ...
    if (!this.state.roster.some(a => a.mapId === this.state.mapId && a.templateId === 'sector_seed_guard')) {
    ...
    this.state.party.forEach(id => {
      const actor = this.state.roster.find(a => a.id === id);
      if (actor) { actor.mapId = this.state.mapId; actor.x = 2 + rand(0,2); actor.y = 2 + rand(0,2); }
    });
  ...
  applyEffects(effects, speakerActor) {
    ...
    if (effects.setFlag) this.state.flags[effects.setFlag] = true;
  ...
  advanceQuest(id, stage) {
    ...
    this.state.quests[id].stage = stage;
    this.log(`Quest updated: ${getById(this.data.quests, id)?.name || id}`);
  }
